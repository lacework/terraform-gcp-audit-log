...
|existing_sink_name|The name of an existing sink which already captures maangement events. **Note:** If both `existing_bucket_name` and `existing_sink_name` are configured, this module must assume they are correctly configured for log capture.|string|""|false
